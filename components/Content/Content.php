...
use Netstars;
use Netstars\Components\RegisteredControl;
use Netstars\Media\Components\Forms;
...
{   
    ...
    {        
        return new Nette\Application\UI\Multiplier(function ($itemId) {
            ...
            $className = 'Netstars\\Media\\Components\\Items\\'.ucfirst($itemChunks[0]);
     ...
     * @param string $name
     * @return \Netstars\Media\Components\SessionManager\PasteBin
     */
...
//    }
    ...
    public function handleInsertFileToContainer($fileId) {
        ...
        $this->presenter->mediaManagerService->loadFile($fileId, $extension['properties'][$this->parent->extName]['mode']);
        
        $folderItem = $this->getFolderContentItem('files', $fileId);
        $section = $this->presenter->mediaManagerService->getFileSection($fileId);
        $restoreUrl = $this->presenter->mediaManagerService->getFileRestoreUrl($folderItem['folder_id'], $fileId, $section, FALSE);
    ...
    public function handleInsertGalleryToContainer($galleryId) {
        ...
        $restoreUrl = $this->presenter->mediaManagerService->getGalleryRestoreUrl($galleryId, $gallery['parent_folder'], TRUE);
    ...
    public function handleInsertGalleryToTiny($galleryId) {
        ...
        $restoreUrl = $this->presenter->mediaManagerService->getGalleryRestoreUrl($galleryId, $gallery['parent_folder'], FALSE);
...
//        $template->actions = $this->actions;
    ...
    public function renderActions() {
        ...
        //dump($config['buttonMenu'][$mediaTrigger]);
